...
class ProductController extends Controller
{
    ...
    public function newlist(Request $request)
    {
        ...
        foreach ($categories as $key => $main) {
            $products = Product::active()->get();
            $product_ids = [];
            foreach ($products as $product) {
                foreach (json_decode($product['category_ids'], true) as $category) {
                    if ($category['id'] == $main->id) {
                        array_push($product_ids, $product['id']);
                    }
                }
            }            
            $products = Product::with('rating')->whereIn('id', $product_ids)->get()->take(6);
            $main['products'] = Helpers::product_data_formatting($products, true);
        }
        return response()->json($categories, 200);
    }
    public function more($categoryId,$offset=0)
    {
        $main = Category::findOrFail($categoryId);
        $products = Product::active()->get();
        $product_ids = [];
        foreach ($products as $product) {
            foreach (json_decode($product['category_ids'], true) as $category) {
        ...
        $products = Product::with('rating')->whereIn('id', $product_ids)->offset($offset)->limit(6)->get();
    ...
    {
         $products = ProductLogic::get_discount_products($request['1'], $request['1']);
        $products['products'] = Helpers::product_data_formatting($products['products'], true);
        return response()->json($products, 200);
    }
}
