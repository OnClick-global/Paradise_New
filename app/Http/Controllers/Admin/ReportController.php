...
class ReportController extends Controller
{
    ...
    public function earning_recipt()
    {
        if (session()->has('from_date') == false) {
            session()->put('from_date', date('Y-m-01'));
            session()->put('to_date', date('Y-m-30'));
        }
        return view('admin-views.report.recipt');
    }

    public function set_date(Request $request)
    {
        session()->put('from_date', date('Y-m-d H:i:s', strtotime($request['from'])));
        session()->put('to_date', date('Y-m-d H:i:s', strtotime($request['to'])));
        return back();
    }
    public function printrecipt(Request $request)
    {
        app()->setlocale('ar');
        ...
        foreach ($orders as $order) {
            ...
            foreach ($order->details as $detail) {
                // dd($order);
                $productsCount += $order->order_amount;
