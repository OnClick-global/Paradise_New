...
class Helpers
{
    public static function compactCharTable($printer, $start = 4, $header = false)
    {
    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['add_ons'] = AddOn::with(['translations'=>function($e){
                    $e->where('locale', app()->getLocale());
                }])->whereIn('id', json_decode($item['add_ons']))->get();
                foreach($item['add_ons'] as $key => $addon ){
                    if(count($addon['translations']) != 0){
                        $addon['name'] = $addon['translations'][0]['value'];
                        unset($addon['translations']);
                    }
            ...
            $attr = [];
            foreach ($data['choice_options'] as $key => $value) {
                $attr = Attribute::with(['translations'=>function($e){
                    $e->where('locale', app()->getLocale());
                }])->where('name',$value->title)->first();
                $value->title = $attr->name;
            }
            $data['add_ons'] = AddOn::with(['translations'=>function($e){
                $e->where('locale', app()->getLocale());
            }])->whereIn('id', json_decode($data['add_ons']))->get();
            foreach($data['add_ons'] as $key => $addon ){
                if(count($addon['translations']) != 0){
                    $addon['name'] = $addon['translations'][0]['value'];
                    unset($addon['translations']);
                }
    ...
    public static function HttpPost($url_path, $data = [])
    {
